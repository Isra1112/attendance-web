...
class AddEmployee extends React.Component {
    ...
    AddEmployee() {
        ...
        axios.post(process.env.REACT_APP_WS_URL + '/employee', body,
            {headers: {
                ...
                <div style={{'marginLeft': '50px', 'marginRight': '50px', 'marginBottom': '100px'}}>
                    {(this.state.error === '') ? <div></div> :
                        (this.state.error === 'error') ?
                            <Message negative>
                                <Message.Header>
                                    <div align="center">Success Add Employee</div>
                                </Message.Header>
                            </Message> :
                            <Message negative>
                                <Message.Header>
                                    <div align="center">Failed Add Employee</div>
                                </Message.Header>
                            </Message>
                    }
                    <Header as='h2'>
                        <Icon name='user plus'/>
                        <Header.Content style={{boxShadow: 'none'}}>New Employee</Header.Content>
                    </Header>
                    <Segment color='red'>
                        <Form>
                            <Form.Field>
                            ...
                            <Form.Group widths="equal">
                                <Form.Field>
                                    <label>QR ID</label>
                                    <input placeholder='QR ID' onChange={e => {
                                        this.setState({qrId: e.target.value})
                                    }}/>
                                </Form.Field>
                                <Form.Field>
                                    <label>NFC ID</label>
                                    <input placeholder='NFC ID' onChange={e => {
                            ...
                            || !this.state.familyContact || !this.state.title || !this.state.dept || !this.state.simperIdCard
                            || !this.state.qrId || !this.state.nfcId} type='submit'
                                    onClick={this.AddEmployee}><Icon name='save outline'></Icon> Submit</Button>
                            <Button onClick={() => {
